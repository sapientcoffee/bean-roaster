...
# Apache-2.0
"""
Opportunistic Dashboard Sync Helper for bean-roaster.
Attempts to find manage_dashboard.py from bean-cup. If present, runs it.
If not present, silently exits 0 so bean-roaster remains 100% self-contained.
"""
import os
import sys
import subprocess
...
def main():
    ...
    if dashboard_script:
        cmd = [sys.executable, dashboard_script] + sys.argv[1:]
        try:
            res = subprocess.run(cmd)
            sys.exit(res.returncode)
        except Exception as e:
            print(f"[bean-roaster] Notice: Dashboard sync skipped ({e})")
            sys.exit(0)
    else:
